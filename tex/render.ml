let cmd_dvips tmpprefix = "/usr/share/texmf/bin/dvips -R -E " ^ tmpprefix ^ ".dvi -f >" ^ tmpprefix ^ ".ps"
...
let cmd_dvipng tmpprefix finalpath = "/usr/share/texmf/bin/dvipng -bg Transparent -fg White -gamma 1.5 -D 120 -T tight --strict " ^ tmpprefix ^ ".dvi -o " ^ finalpath ^ " >/dev/null 2>/dev/null"
